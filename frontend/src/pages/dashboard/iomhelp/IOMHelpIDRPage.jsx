/**
 * IOMHelpIDRPage.jsx — IDR Payment Summary tab for IOM Help Dashboard.
 *
...
export default function IOMHelpIDRPage() {
  ...
  const { data: trendRaw  = [], isLoading: trendLoad  } = useQuery({
    queryKey: ['ionm-idr-trend'],
    queryFn:  () => ionmApi.getIdrPaymentTrend().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  });
  const { data: statusRaw = [], isLoading: statusLoad } = useQuery({
    queryKey: ['ionm-idr-status'],
    queryFn:  () => ionmApi.getIdrStatusCount().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  });
  const { data: protechRaw = [], isLoading: ptLoad } = useQuery({
    queryKey: ['ionm-idr-protech'],
    queryFn:  () => ionmApi.getIdrProTech().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  });
  const { data: insRaw = [], isLoading: insLoad } = useQuery({
    queryKey: ['ionm-idr-insurance'],
    queryFn:  () => ionmApi.getIdrInsurance().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  });
