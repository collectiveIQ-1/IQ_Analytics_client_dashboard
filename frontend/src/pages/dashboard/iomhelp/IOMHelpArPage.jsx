/**
 * IOMHelpArPage.jsx — Accounts Receivable tab for IOM Help Dashboard.
 *
...
export default function IOMHelpArPage() {
  ...
  const { data: dosData, isLoading: dosLoad } = useQuery({
    queryKey: ['ionm-ar-dos'],
    queryFn:  () => ionmApi.getArDos().then((r) => r.data.data),
    staleTime: 300_000,
    enabled: view === 'dos',
  });

  const { data: doeData, isLoading: doeLoad } = useQuery({
    queryKey: ['ionm-ar-doe'],
    queryFn:  () => ionmApi.getArDoe().then((r) => r.data.data),
    staleTime: 300_000,
    enabled: view === 'doe',
  });

  const { data: insData, isLoading: insLoad } = useQuery({
    queryKey: ['ionm-ar-insurance', view],
    queryFn:  () => ionmApi.getArInsurance(view).then((r) => r.data.data),
    staleTime: 300_000,
    enabled: expanded && expandTab === 'insurance',
  });

  const { data: surgData, isLoading: surgLoad } = useQuery({
    queryKey: ['ionm-ar-surgeon', view],
    queryFn:  () => ionmApi.getArSurgeon(view).then((r) => r.data.data),
    staleTime: 300_000,
