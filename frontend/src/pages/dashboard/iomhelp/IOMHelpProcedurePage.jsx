/**
 * IOMHelpProcedurePage.jsx — Procedure tab for IOM Help Dashboard.
 *
...
export default function IOMHelpProcedurePage() {
  ...
  const { data: depositsRaw, isLoading: depLoad } = useQuery({
    queryKey: ['ionm-proc-deposits'],
    queryFn:  () => ionmApi.getProcedureDeposits().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  });

  const { data: chargesRaw, isLoading: chrLoad } = useQuery({
    queryKey: ['ionm-proc-charges', view],
    queryFn:  () => ionmApi.getProcedureCharges(view).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  });
  ...
  const { data: dosMoreRaw, isLoading: dosMoreLoad, isFetching: dosMoreFetch } = useQuery({
    queryKey: ['ionm-proc-more', 'dos'],
    queryFn:  () => ionmApi.getProcedureMore('dos').then((r) => r.data?.data ?? r.data ?? []),
    enabled:  showMore === 'charges',
    staleTime: 5 * 60_000,
  });

  const { data: doeMoreRaw, isLoading: doeMoreLoad, isFetching: doeMoreFetch } = useQuery({
    queryKey: ['ionm-proc-more', 'doe'],
    queryFn:  () => ionmApi.getProcedureMore('doe').then((r) => r.data?.data ?? r.data ?? []),
    enabled:  showMore === 'charges',
    staleTime: 5 * 60_000,
  });
  ...
  const { data: dodMoreRaw, isLoading: dodMoreLoad, isFetching: dodMoreFetch } = useQuery({
    queryKey: ['ionm-proc-dod-more'],
    queryFn:  () => ionmApi.getProcedureDodMore().then((r) => r.data?.data ?? r.data ?? []),
