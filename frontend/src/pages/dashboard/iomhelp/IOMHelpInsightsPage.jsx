/**
 * IOMHelpInsightsPage.jsx — Insights tab for IOM Help Dashboard.
 *
...
export default function IOMHelpInsightsPage() {
  ...
  const { data: insuranceData, isLoading: insLoad } = useQuery({
    queryKey: ['ionm-insights-insurance'],
    queryFn:  () => ionmApi.getInsightsInsurance().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    enabled: view === 'insurance',
  });
  const { data: surgeonData, isLoading: surLoad } = useQuery({
    queryKey: ['ionm-insights-surgeon', filter],
    queryFn:  () => ionmApi.getInsightsSurgeon(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    enabled: view === 'surgeon',
  });
  const { data: readerData, isLoading: rdLoad } = useQuery({
    queryKey: ['ionm-insights-reader', filter],
    queryFn:  () => ionmApi.getInsightsReader(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    enabled: view === 'reader',
  });
  const { data: techData, isLoading: techLoad } = useQuery({
    queryKey: ['ionm-insights-technician', filter],
    queryFn:  () => ionmApi.getInsightsTechnician(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  ...
  const { data: surgeonList = [] } = useQuery({
    queryKey: ['ionm-insights-surgeon-list'],
    queryFn:  () => ionmApi.getInsightsSurgeonList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    enabled: view === 'surgeon',
  });
  const { data: readerList = [] } = useQuery({
    queryKey: ['ionm-insights-reader-list'],
    queryFn:  () => ionmApi.getInsightsReaderList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    enabled: view === 'reader',
  });
  const { data: techList = [] } = useQuery({
    queryKey: ['ionm-insights-tech-list'],
    queryFn:  () => ionmApi.getInsightsTechList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    enabled: view === 'technician',
  });
