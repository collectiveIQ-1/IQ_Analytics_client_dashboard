/**
 * IOMHelpCleanClaimPage.jsx — CCR detail page for IOM Help.
 *
...
export default function IOMHelpCleanClaimPage({ onNavigate }) {
  const { data: kpis,       isLoading: kpiLoading    } = useQuery({
    queryKey: ['ionm-kpis'],
    queryFn:  () => ionmApi.getKpis().then(r => r.data.data),
    staleTime: 300_000,
  });
  const { data: ccrData,    isLoading: ccrLoading    } = useQuery({
    queryKey: ['ionm-ccr-history'],
    queryFn:  () => ionmApi.getCcrHistory().then(r => r.data.data),
    staleTime: 300_000,
  });
  const { data: denialData, isLoading: denialLoading } = useQuery({
    queryKey: ['ionm-denial-reasons'],
    queryFn:  () => ionmApi.getDenialReasons().then(r => r.data.data),
    staleTime: 300_000,
  });
