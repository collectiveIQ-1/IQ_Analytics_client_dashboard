/**
 * IOMHelpPaymentsPage.jsx — Payments tab for IOM Help Dashboard.
 *
 * Layout:
...
export default function IOMHelpPaymentsPage() {
  ...
  const { data: lineData, isLoading: lineLoad } = useQuery({
    queryKey: ['ionm-pay-line'],
    queryFn:  () => ionmApi.getPaymentLineChart().then(r => r.data.data),
    staleTime: 300_000,
  });

  const { data: surgeonData, isLoading: surgeonLoad } = useQuery({
    queryKey: ['ionm-pay-surgeon'],
    queryFn:  () => ionmApi.getDepositsBySurgeon().then(r => r.data.data),
    staleTime: 300_000,
    enabled: chartTab === 'surgeon',
  });

  const { data: hospitalData, isLoading: hospitalLoad } = useQuery({
    queryKey: ['ionm-pay-hospital'],
    queryFn:  () => ionmApi.getDepositsByHospital().then(r => r.data.data),
    staleTime: 300_000,
    enabled: chartTab === 'hospital',
  });

  const { data: billingTypeData, isLoading: billingTypeLoad } = useQuery({
    queryKey: ['ionm-pay-billing-type'],
    queryFn:  () => ionmApi.getDepositsByBillingType().then(r => r.data.data),
    staleTime: 300_000,
    enabled: chartTab === 'billing_type',
  });

  const { data: insuranceData, isLoading: insuranceLoad } = useQuery({
    queryKey: ['ionm-pay-insurance'],
    queryFn:  () => ionmApi.getDepositsByInsurance().then(r => r.data.data),
    staleTime: 300_000,
  ...
  return (
    <div className="space-y-5">
      ...
      {/* ── Section 1: All Time Payment History line chart ── */}
      <ChartCard
        title="All Time Payment History"
        loading={lineLoad}
        height={340}
      >
