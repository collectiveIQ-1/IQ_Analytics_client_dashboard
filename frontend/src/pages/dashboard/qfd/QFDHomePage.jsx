/**
 * QFDHomePage.jsx — QFD Dashboard Home tab.
 *
 * Layout (3-row grid, single viewport, no scroll):
 *  Row 1: Revenue Cycle Metrics (col-7) | Payment History (col-5)
 *  Row 2: Charges vs Payments (col-6) | CCR History (col-3) | AR Receivable (col-3)
 *  Row 3: Total Charges (col-4) | Total Adjustments (col-4) | AR% >60 Days (col-4)
 *
 * Global cross-chart interaction:
 *   • selectedDate  — clicking CCR bar or Charges bar highlights that month across all charts
 *   • selectedArBucket — clicking AR pie or donut slice highlights across both
 *
 * Expand behaviour: each row expands independently.
 */
...
// ── CCR bar label — inside bar to avoid clipping ──────────────────────────────
function CcrBarLabel({ x, y, width, height, value }) {
  if (!value || height < 20) return null;
  // Show label inside the bar when tall enough, otherwise above with adequate margin
  if (height >= 30) {
    return (
      <text
        x={x + width / 2}
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button onClick={onClear}
        className="text-[10px] text-slate-400 hover:text-red-500 dark:text-zinc-500">
        × Clear
      </button>
    </div>
  );
...
// ── Component ─────────────────────────────────────────────────────────────────
export default function QFDHomePage({ onNavigate }) {
  ...
  // ── Data queries ──────────────────────────────────────────────────────────
  const { data: kpis,        isLoading: kpiLoad        } = useQuery({ queryKey:['qfd-kpis'],                      queryFn:()=>qfdApi.getKpis().then(r=>r.data.data),                staleTime:300_000 });
  const { data: paymentData, isLoading: payLoad         } = useQuery({ queryKey:['qfd-payment-history'],           queryFn:()=>qfdApi.getPaymentHistory().then(r=>r.data.data),       staleTime:300_000 });
  const { data: paymentFull, isLoading: payFullLoad     } = useQuery({ queryKey:['qfd-payment-history-full'],      queryFn:()=>qfdApi.getPaymentHistoryFull().then(r=>r.data.data),   staleTime:300_000, enabled: row1Expanded==='payment' });
  const { data: ccrData,     isLoading: ccrLoad         } = useQuery({ queryKey:['qfd-ccr-history'],               queryFn:()=>qfdApi.getCcrHistory().then(r=>r.data.data),           staleTime:300_000 });
  const { data: arPieData,   isLoading: arPieLoad       } = useQuery({ queryKey:['qfd-ar-pie'],                    queryFn:()=>qfdApi.getArPie().then(r=>r.data.data),                staleTime:300_000 });
  const { data: donutData,   isLoading: donutLoad       } = useQuery({ queryKey:['qfd-ar-donut'],                  queryFn:()=>qfdApi.getArDonut().then(r=>r.data.data),              staleTime:300_000 });
  const { data: tcData,      isLoading: tcLoad          } = useQuery({ queryKey:['qfd-total-charges'],             queryFn:()=>qfdApi.getTotalCharges().then(r=>r.data.data),         staleTime:300_000 });
  const { data: tcFull,      isLoading: tcFullLoad      } = useQuery({ queryKey:['qfd-total-charges-full'],        queryFn:()=>qfdApi.getTotalChargesFull().then(r=>r.data.data),     staleTime:300_000, enabled: row3Expanded==='totalCharges' });
  const { data: adjData,     isLoading: adjLoad         } = useQuery({ queryKey:['qfd-adjustments'],               queryFn:()=>qfdApi.getAdjustments().then(r=>r.data.data),          staleTime:300_000 });
  const { data: adjFull,     isLoading: adjFullLoad     } = useQuery({ queryKey:['qfd-adjustments-full'],          queryFn:()=>qfdApi.getAdjustmentsFull().then(r=>r.data.data),      staleTime:300_000, enabled: row3Expanded==='adjustments' });
  const { data: chargesData, isLoading: chargesLoad     } = useQuery({ queryKey:['qfd-charges-vs-payments'],       queryFn:()=>qfdApi.getChargesVsPayments().then(r=>r.data.data),    staleTime:300_000 });
  const { data: chargesFull, isLoading: chargesFullLoad } = useQuery({ queryKey:['qfd-charges-vs-payments-full'],  queryFn:()=>qfdApi.getChargesVsPaymentsFull().then(r=>r.data.data),staleTime:300_000, enabled: row2Expanded==='charges' });
    ...
    <div className="flex flex-col gap-3">
      ...
      {(selectedDate || selectedArBucket) && (
        <div className="flex items-center gap-4 px-2 py-1.5 rounded-xl bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 text-[10px]">
          ...
          {(selectedDate || selectedArBucket) && (
            <button onClick={() => { setSelectedDate(null); setSelectedArBucket(null); }}
      ...
      <div className="grid grid-cols-12 gap-3">
        ...
        <div className={`${r1MetricsClass} transition-all duration-300`}>
          <div className="bg-white dark:bg-zinc-950 border border-slate-100 dark:border-zinc-800 rounded-2xl shadow-sm h-full">
            ...
            <div className="p-3 grid grid-cols-2 gap-3 h-[calc(100%-44px)]">
              ...
              <QFDKpiCard label="Clean Claim Rate" value={fmtPct(kpis?.ccr)}                 subLabel="Last Month"     color="purple" loading={kpiLoad} onClick={() => onNavigate?.('ccr')} />
            </div>
          </div>
        </div>
        ...
        <div className={`${r1PayClass} transition-all duration-300`}>
          <QFDChartCard
            title={row1Expanded === 'payment' ? 'All Time Payment History (DOD)' : 'Last 12 Months Payment History (DOD)'}
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Line
                  ...
                  strokeWidth={2}
                  dot={({ cx, cy, payload }) => {
                    const isHighlighted = selectedDate && payload?.date === selectedDate;
                    return (
                      <circle key={payload?.date} cx={cx} cy={cy}
                        r={isHighlighted ? 5 : selectedDate ? 2 : 3}
                        fill={isHighlighted ? '#b91c1c' : '#dc2626'}
                        opacity={selectedDate && !isHighlighted ? 0.4 : 1}
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                ...
                style={{ cursor: 'pointer' }}
                onClick={(state) => {
                  if (state?.activeLabel) toggleDate(state.activeLabel);
                }}
              >
                <XAxis dataKey="date" tick={{ fontSize: 9 }} tickFormatter={fmtShortDate} />
                <YAxis tick={{ fontSize: 9 }} tickFormatter={fmtMoney} width={48} />
                ...
                <Bar dataKey="total_charges"  name="Total Charge"   fill="#1e3a5f" radius={[2,2,0,0]} maxBarSize={22}>
                  {(activeCharges || []).map((entry, i) => (
                    <Cell key={i}
                      fill="#1e3a5f"
                      opacity={selectedDate && entry.date !== selectedDate ? 0.35 : 1}
                    />
                  ))}
                </Bar>
              ...
              >
                ...
                <Bar dataKey="adjusted" name="CCR" radius={[2,2,0,0]} maxBarSize={20}>
                  {(ccrData || []).map((entry, i) => (
                    <Cell key={i}
                      fill="#dc2626"
                      opacity={selectedDate && entry.month !== selectedDate ? 0.35 : 1}
                    />
                  ))}
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Line type="stepAfter" dataKey="total_charges" name="Total Charges" stroke="#7c3aed"
                  strokeWidth={2}
                  dot={({ cx, cy, payload }) => {
                    const isHighlighted = selectedDate && payload?.date === selectedDate;
                    return (
                      <circle key={payload?.date} cx={cx} cy={cy}
                        r={isHighlighted ? 5 : 3}
                        fill="#7c3aed"
                        opacity={selectedDate && !isHighlighted ? 0.35 : 1}
                        stroke={isHighlighted ? '#fff' : 'none'}
                        strokeWidth={isHighlighted ? 2 : 0}
                      />
                    );
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Area type="monotone" dataKey="adjustments" name="Adjustments"
                  stroke="#ca8a04" strokeWidth={2} fill="url(#adjGrad)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </QFDChartCard>
        </div>
