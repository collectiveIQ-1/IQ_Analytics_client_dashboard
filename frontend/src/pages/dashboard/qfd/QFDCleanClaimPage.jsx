/**
 * QFDCleanClaimPage.jsx — Clean Claim Rate detail page.
 *
 * Contains:
 *   A) Revenue Cycle Metrics (same 4 KPI cards)
...
export default function QFDCleanClaimPage({ onNavigate }) {
  // ── Data ──────────────────────────────────────────────────────────────────────

  const { data: kpis, isLoading: kpiLoading } = useQuery({
    queryKey: ['qfd-kpis'],
    queryFn:  () => qfdApi.getKpis().then((r) => r.data.data),
    staleTime: 5 * 60_000,
  });

  const { data: ccrData, isLoading: ccrLoading } = useQuery({
    queryKey: ['qfd-ccr-history'],
    queryFn:  () => qfdApi.getCcrHistory().then((r) => r.data.data),
    staleTime: 5 * 60_000,
  });

  const { data: denialData, isLoading: denialLoading } = useQuery({
    queryKey: ['qfd-denial-reasons'],
    queryFn:  () => qfdApi.getDenialReasons().then((r) => r.data.data),
    staleTime: 5 * 60_000,
  });
