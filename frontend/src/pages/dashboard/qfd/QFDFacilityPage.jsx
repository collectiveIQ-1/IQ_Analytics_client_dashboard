/**
 * QFDFacilityPage.jsx — Facility Analysis tab for QFD Dashboard.
 *
 * Tabs: DOS | DOE | DOD
 ...
 *
 * Fixes applied:
 *   • staleTime: 0  — data is always treated as stale so it re-fetches on
 *     every mount / window-focus instead of serving a cached empty response.
 *   • retry: 3 with exponential back-off — if the DB is temporarily
 *     unreachable the query retries automatically instead of caching "no data".
 *   • Error state with Retry button — distinguishes "DB unavailable" from
 *     "genuinely no data", giving the user a one-click way to recover.
 */
...
// ── Facility group (one facility block) ───────────────────────────────────────

function FacilityGroup({ row, maxVal, colors, isLast }) {
  const metrics = [
...
// ── Facility chart component ──────────────────────────────────────────────────

function FacilityChart({ data, colors, loading, error, onRetry }) {
...
// ── Chart card wrapper ────────────────────────────────────────────────────────
// Fixed min-height so both cards stay the same size regardless of data count.
// Content area scrolls internally when data exceeds the visible height.
...
function ChartCard({ title, tag, month, months, onMonthChange, children }) {
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 pt-4 pb-2 gap-3 flex-none">
        ...
        {months != null && (
          <div className="flex items-center gap-1.5 flex-shrink-0">
            ...
            >
              {tag === 'DOE' && <option value="all">(All)</option>}
              {months.map((m) => (
      ...
      {/* Fixed height content area — scrolls internally so card never changes size */}
...
// staleTime:0 ensures we never serve a stale cached empty response.
// retry:3 with exponential back-off recovers automatically when the DB is
// temporarily unavailable (e.g. backend just restarted).
...
export default function QFDFacilityPage() {
  ...
  const { data: dosL12Raw, isLoading: dosL12Load, error: dosL12Err } = useQuery({
    ...Q,
    queryKey: ['facility-dos-last12', dosMonth],
    queryFn:  () => facilityApi.getDosLast12(dosMonth).then((r) => r.data.data),
    enabled:  mode === 'dos',
  });

  const { data: dosLMRaw, isLoading: dosLMLoad, error: dosLMErr } = useQuery({
    ...Q,
    queryKey: ['facility-dos-lastmonth'],
    queryFn:  () => facilityApi.getDosLastMonth().then((r) => r.data.data),
    enabled:  mode === 'dos',
  });
  ...
  const { data: doeL12Raw, isLoading: doeL12Load, error: doeL12Err } = useQuery({
    ...Q,
    queryKey: ['facility-doe-last12', doeMonth],
    queryFn:  () => facilityApi.getDoeLast12(doeMonth).then((r) => r.data.data),
    enabled:  mode === 'doe',
  });

  const { data: doeLMRaw, isLoading: doeLMLoad, error: doeLMErr } = useQuery({
    ...Q,
    queryKey: ['facility-doe-lastmonth'],
    queryFn:  () => facilityApi.getDoeLastMonth().then((r) => r.data.data),
    enabled:  mode === 'doe',
  });
  ...
  const { data: dodL12Raw, isLoading: dodL12Load, error: dodL12Err } = useQuery({
    ...Q,
    queryKey: ['facility-dod-last12', dodMonth],
    queryFn:  () => facilityApi.getDodLast12(dodMonth).then((r) => r.data.data),
    enabled:  mode === 'dod',
  });

  const { data: dodLMRaw, isLoading: dodLMLoad, error: dodLMErr } = useQuery({
    ...Q,
    queryKey: ['facility-dod-lastmonth'],
    queryFn:  () => facilityApi.getDodLastMonth().then((r) => r.data.data),
    enabled:  mode === 'dod',
  });
  ...
  const retryL12 = () => queryClient.invalidateQueries({
    queryKey: isDos ? ['facility-dos-last12', dosMonth] : isDod ? ['facility-dod-last12', dodMonth] : ['facility-doe-last12', doeMonth],
  });
  const retryLM = () => queryClient.invalidateQueries({
    queryKey: isDos ? ['facility-dos-lastmonth'] : isDod ? ['facility-dod-lastmonth'] : ['facility-doe-lastmonth'],
  });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-end gap-2">
        <ViewBtn active={mode === 'doe'} onClick={() => setMode('doe')}>DOE</ViewBtn>
        <ViewBtn active={mode === 'dos'} onClick={() => setMode('dos')}>DOS</ViewBtn>
        <ViewBtn active={mode === 'dod'} onClick={() => setMode('dod')}>DOD</ViewBtn>
      </div>
