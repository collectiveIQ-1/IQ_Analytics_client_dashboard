/**
 * NeuroWatchPaymentsPage.jsx — Payments tab for Neuro Watch Dashboard.
 *
 * Section 1: All Time Payment History (line chart)
 * Section 2: Deposits by [Surgeon | Hospital | Billing Type | Insurance Type]
 */
...
function ChartCard({ title, actions, height = 340, loading, children }) {
  return (
    <div
      ...
      className="overflow-hidden rounded-2xl border border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-950 shadow-card"
    >
...
export default function NeuroWatchPaymentsPage() {
  ...
  const { data: lineData, isLoading: lineLoad } = useQuery({
    queryKey: ['nw-pay-line'],
    queryFn:  () => neurowatchApi.getPaymentLineChart().then(r => r.data.data),
    staleTime: 300_000,
  });

  const { data: surgeonData,     isLoading: surgeonLoad     } = useQuery({ queryKey: ['nw-pay-surgeon'],      queryFn: () => neurowatchApi.getDepositsBySurgeon().then(r => r.data.data),     staleTime: 300_000, enabled: chartTab === 'surgeon' });
  const { data: hospitalData,    isLoading: hospitalLoad    } = useQuery({ queryKey: ['nw-pay-hospital'],     queryFn: () => neurowatchApi.getDepositsByHospital().then(r => r.data.data),    staleTime: 300_000, enabled: chartTab === 'hospital' });
  const { data: billingTypeData, isLoading: billingTypeLoad } = useQuery({ queryKey: ['nw-pay-billing-type'],queryFn: () => neurowatchApi.getDepositsByBillingType().then(r => r.data.data), staleTime: 300_000, enabled: chartTab === 'billing_type' });
  const { data: insuranceData,   isLoading: insuranceLoad   } = useQuery({ queryKey: ['nw-pay-insurance'],   queryFn: () => neurowatchApi.getDepositsByInsurance().then(r => r.data.data),   staleTime: 300_000, enabled: chartTab === 'insurance' });
  ...
  return (
    <div className="space-y-5">
      ...
      <ChartCard title="All Time Payment History" loading={lineLoad} height={340}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={lineData || []} margin={{ top: 20, right: 30, left: 10, bottom: 10 }}>
            ...
            <YAxis tickFormatter={fmtMoney} tick={{ fontSize: 10, fill: '#64748b' }} width={56} axisLine={false} tickLine={false} />
      ...
      <ChartCard
        title={'Deposits by ' + (CHART_TABS.find(t => t.id === chartTab)?.label || '')}
        ...
        <div style={{ height: barHeight, overflowY: 'auto' }}>
          <ResponsiveContainer width="100%" height={barHeight}>
            ...
            >
              ...
              <YAxis dataKey="label" type="category" width={leftMargin - 10} tick={{ fontSize: 10, fill: axisFill, fontWeight: 600 }} axisLine={false} tickLine={false} />
              <Tooltip content={<BarTooltip />} />
              <Bar dataKey="payments" name="Payment Collected" fill={active.color} radius={[0, 3, 3, 0]} maxBarSize={28}>
