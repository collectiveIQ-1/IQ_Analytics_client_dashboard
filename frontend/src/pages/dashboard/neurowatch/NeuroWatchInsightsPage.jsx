/**
 * NeuroWatchInsightsPage.jsx — Insights tab for Neuro Watch Dashboard.
 *
 * Toggle: Insurance | Surgeon | Reader | Technician
 * Table:  entity × billing_date month columns, 6 metrics per entity row
 *
 * % Total Payments = SUM(collected) / monthly total × 100  (window fn on backend)
 */
...
const METRICS = [
  { key: 'visit_count',        label: 'Visit Count',         fmt: fmtCount },
...
function Skeleton({ h = 200 }) {
  return (
    <div
      className="animate-pulse bg-slate-100 dark:bg-zinc-800 rounded-lg"
      style={{ height: h }}
    />
  );
}
      ...
      <table className="min-w-full text-xs border-collapse">
        <thead>
          <tr className="bg-slate-50 dark:bg-zinc-800 border-b border-slate-200 dark:border-zinc-700">
            <th className="sticky left-0 z-10 bg-slate-50 dark:bg-zinc-800 px-3 py-2.5 text-left font-semibold text-slate-600 dark:text-zinc-300 min-w-[140px]">
              {entityLabel}
            </th>
            <th className="px-3 py-2.5 text-left font-semibold text-slate-500 dark:text-zinc-400 min-w-[140px]" />
            {months.map((m) => (
              <th
                key={m}
                className="px-3 py-2.5 text-right font-semibold text-slate-600 dark:text-zinc-300 whitespace-nowrap min-w-[100px]"
              >
                {fmtMonth(m)}
              </th>
            ))}
          ...
          {Object.entries(entities).map(([entity, monthMap], eIdx) =>
            METRICS.map((row, rowIdx) => {
              ...
              return (
                ...
                >
                  <td
                    className={`sticky left-0 z-10 ${stickyBg} px-3 py-1 font-semibold text-slate-700 dark:text-zinc-200 align-top border-r border-slate-100 dark:border-zinc-700`}
                  >
                    {isFirst ? entity : ''}
                  </td>
                  ...
                  {months.map((m) => {
                    ...
                    return (
                      <td
                        key={m}
                        className="px-3 py-1 text-right tabular-nums text-slate-600 dark:text-zinc-300 whitespace-nowrap"
                      >
                        {row.fmt(val)}
                      </td>
                    );
...
export default function NeuroWatchInsightsPage() {
  ...
  const { data: insuranceData, isLoading: insLoad } = useQuery({
    queryKey:  ['nw-insights-insurance'],
    queryFn:   () => neurowatchApi.getInsightsInsurance().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    retry:     false,
    enabled:   view === 'insurance',
  });
  const { data: surgeonData, isLoading: surLoad } = useQuery({
    queryKey:  ['nw-insights-surgeon', filter],
    queryFn:   () => neurowatchApi.getInsightsSurgeon(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    retry:     false,
    enabled:   view === 'surgeon',
  });
  const { data: readerData, isLoading: rdLoad } = useQuery({
    queryKey:  ['nw-insights-reader', filter],
    queryFn:   () => neurowatchApi.getInsightsReader(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    retry:     false,
    enabled:   view === 'reader',
  });
  const { data: techData, isLoading: techLoad } = useQuery({
    queryKey:  ['nw-insights-technician', filter],
    queryFn:   () => neurowatchApi.getInsightsTechnician(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    retry:     false,
  ...
  const { data: surgeonList = [] } = useQuery({
    queryKey:  ['nw-insights-surgeon-list'],
    queryFn:   () => neurowatchApi.getInsightsSurgeonList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    retry:     false,
    enabled:   view === 'surgeon',
  });
  const { data: readerList = [] } = useQuery({
    queryKey:  ['nw-insights-reader-list'],
    queryFn:   () => neurowatchApi.getInsightsReaderList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    retry:     false,
    enabled:   view === 'reader',
  });
  const { data: techList = [] } = useQuery({
    queryKey:  ['nw-insights-tech-list'],
    queryFn:   () => neurowatchApi.getInsightsTechList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    retry:     false,
    enabled:   view === 'technician',
  });
