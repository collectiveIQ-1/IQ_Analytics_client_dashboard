/**
 * InnervateProductionPage.jsx — Productions tab for Innervate Dashboard.
 *
 * Mirrors IOM Help Production Page exactly.
 * No filters — raw Innervate table data only.
 *
 * Views (toggled by DOE / DOS / DOD buttons top-right):
 *   DOS — Grouped bar: Total Charges (purple) vs Payments (amber) + GCR% label
 *         More → Reimbursement Analysis table (DOS months)
 *   DOE — Single blue bar chart: Total Charges by billing_date
 *         More → Reimbursement Analysis table (DOE months)
 *   DOD — 2×2 grid:
 *         Top-left:  Last 12 Months Adjustment History (area — billing)
 *         Top-right: Deposit by Payer (h-bar — deposit)
 *         Bot-left:  Last 12 Months Payments History (line — deposit)
 *         Bot-right: Deposit by Billing Entity (h-bar — deposit)
 *
 * Schema : iq_innervate
 * Tables : innervate_full_billing (DOS/DOE/Reimb/Adj), innervate_full_deposit (DOD)
 */
        ...
        <tbody>
          {rowDefs.map((def, ri) => (
            ...
            >
              ...
              >
                {def.indent > 0 && <span className="text-slate-300 dark:text-zinc-600 mr-1 text-[10px]">└</span>}
                {def.label}
              </td>
              {data.map((row, ci) => (
...
export default function InnervateProductionPage() {
  ...
  const { data: dosChart,   isLoading: dosLoad      } = useQuery({ queryKey: ['inv-prod-dos-chart'],  queryFn: () => innervateApi.getProductionDosChart().then(r => r.data.data),    staleTime: 300_000, enabled: view === 'dos' && !showMore });
  const { data: doeChart,   isLoading: doeLoad      } = useQuery({ queryKey: ['inv-prod-doe-chart'],  queryFn: () => innervateApi.getProductionDoeChart().then(r => r.data.data),    staleTime: 300_000, enabled: view === 'doe' && !showMore });
  const { data: dosTable,   isLoading: dosTblLoad   } = useQuery({ queryKey: ['inv-prod-dos-reimb'],  queryFn: () => innervateApi.getProductionDosReimb().then(r => r.data.data),    staleTime: 300_000, enabled: view === 'dos' && showMore });
  const { data: doeTable,   isLoading: doeTblLoad   } = useQuery({ queryKey: ['inv-prod-doe-reimb'],  queryFn: () => innervateApi.getProductionDoeReimb().then(r => r.data.data),    staleTime: 300_000, enabled: view === 'doe' && showMore });
  const { data: adjData,    isLoading: adjLoad      } = useQuery({ queryKey: ['inv-prod-dod-adj'],    queryFn: () => innervateApi.getProductionDodAdjustments().then(r => r.data.data), staleTime: 300_000, enabled: view === 'dod' });
  const { data: pmtData,    isLoading: pmtLoad      } = useQuery({ queryKey: ['inv-prod-dod-pmt'],    queryFn: () => innervateApi.getProductionDodPayments().then(r => r.data.data),  staleTime: 300_000, enabled: view === 'dod' });
  const { data: payerData,  isLoading: payerLoad    } = useQuery({ queryKey: ['inv-prod-dod-payer'],  queryFn: () => innervateApi.getProductionDodByPayer().then(r => r.data.data),   staleTime: 300_000, enabled: view === 'dod' });
  const { data: billerData, isLoading: billerLoad   } = useQuery({ queryKey: ['inv-prod-dod-biller'], queryFn: () => innervateApi.getProductionDodByBillerEntity().then(r => r.data.data), staleTime: 300_000, enabled: view === 'dod' });
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm overflow-hidden">
        ...
        {view === 'dos' && !showMore && (
          <>
              ...
              {dosLoad ? <ChartSkeleton height={380} /> : (
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    />
                    <YAxis
                      tickFormatter={fmtMoney} width={58}
                      tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false}
            ...
            <div className="flex justify-end px-4 pb-4">
              <button onClick={() => setShowMore(true)} className="px-5 py-2 rounded-lg bg-red-700 text-white text-sm font-bold hover:bg-red-800 transition-colors">
                More
              </button>
            </div>
          </>
              ...
              {doeLoad ? <ChartSkeleton height={380} /> : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={doeChart || []} margin={{ top: 30, right: 20, left: 10, bottom: 10 }}>
                    <XAxis dataKey="date" tickFormatter={fmtShortDate} tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                    <YAxis tickFormatter={fmtMoney} width={58} tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                    <Tooltip content={<ChartTooltip />} />
                    <Bar dataKey="total_charges" name="Total Charges" fill="#1E3A8A" radius={[3,3,0,0]} maxBarSize={80}>
                      <LabelList dataKey="total_charges" position="insideBottom" offset={12} angle={-90} fontSize={10} fontWeight="700" fill="white" formatter={fmtMoney} />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              )}
        ...
        {/* ══ DOS / DOE Reimbursement table ══ */}
        {(view === 'dos' || view === 'doe') && showMore && (
          <>
            <div className="px-2 md:px-4 pb-2">
              <ReimbursementTable data={activeTable} loading={tableFetching} rows={activeRows} />
            </div>
            <div className="flex justify-end px-4 pb-4">
              <button onClick={() => setShowMore(false)} className="px-5 py-2 rounded-lg bg-red-700 text-white text-sm font-bold hover:bg-red-800 transition-colors">
                Go Back
              </button>
            </div>
          </>
        ...
        {view === 'dod' && (
          <div className="px-4 pb-4 space-y-4">
                  ...
                  {adjLoad ? <ChartSkeleton height={240} /> : (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={adjData || []} margin={{ top: 28, right: 20, left: 10, bottom: 10 }}>
                        <defs>
                          <linearGradient id="invAdjGrad" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="5%"  stopColor="#eab308" stopOpacity={0.6} />
                            <stop offset="95%" stopColor="#eab308" stopOpacity={0.05} />
                          </linearGradient>
                        </defs>
                        <XAxis dataKey="date" tickFormatter={fmtShortDate} tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <YAxis tickFormatter={fmtMoney} width={52} tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <Tooltip content={<ChartTooltip />} />
                        <Area type="monotone" dataKey="adjustments" name="Adjustments" stroke="#D97706" strokeWidth={2} fill="url(#invAdjGrad)" dot={false}>
              ...
              <div>
                ...
                <div style={{ minHeight: 240, height: 240 }}>
                  {payerLoad ? <ChartSkeleton height={240} /> : (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={payerData || []} layout="vertical" margin={{ top: 5, right: 90, left: 10, bottom: 5 }}>
                        <XAxis type="number" tickFormatter={fmtMoney} tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <YAxis dataKey="payer" type="category" width={140} tick={{ fontSize: 9, fill: '#334155', fontWeight: 600 }} axisLine={false} tickLine={false} />
                        <Tooltip content={<ChartTooltip />} />
            ...
            {/* Row 2: Payments History + Biller Entity */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {pmtLoad ? <ChartSkeleton height={240} /> : (
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={pmtData || []} margin={{ top: 28, right: 20, left: 10, bottom: 10 }}>
                        ...
                        <YAxis tickFormatter={fmtMoney} width={52} tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <Tooltip content={<ChartTooltip />} />
                        <Line type="monotone" dataKey="payments" name="Payments" stroke="#EF4444" strokeWidth={2.5} dot={{ r: 3, fill: '#EF4444', strokeWidth: 0 }} activeDot={{ r: 5 }}>
              ...
              <div>
                ...
                <div style={{ minHeight: 240, height: 240 }}>
                  {billerLoad ? <ChartSkeleton height={240} /> : (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={billerData || []} layout="vertical" margin={{ top: 5, right: 90, left: 10, bottom: 5 }}>
                        <XAxis type="number" tickFormatter={fmtMoney} tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <YAxis dataKey="biller_entity" type="category" width={150} tick={{ fontSize: 9, fill: '#334155', fontWeight: 600 }} axisLine={false} tickLine={false} />
                        <Tooltip content={<ChartTooltip />} />
