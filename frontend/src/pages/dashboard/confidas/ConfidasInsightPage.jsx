/**
 * ConfidasInsightPage.jsx — Insight Analysis page for Confidas Dashboard.
 *
...
// ── Main component ────────────────────────────────────────────────────────────
export default function ConfidasInsightPage() {
  ...
  const { data: dosFiltersRaw } = useQuery({
    ...Q,
    queryKey:  ['conf-insight-dos-filters', groupId],
    queryFn:   () => confidasApi.getInsightDosFilters(groupId).then((r) => r.data.data?.values || []),
    staleTime: 5 * 60_000,
    enabled:   needsFilter && mode === 'dos',
  });

  const { data: doeFiltersRaw } = useQuery({
    ...Q,
    queryKey:  ['conf-insight-doe-filters', groupId],
    queryFn:   () => confidasApi.getInsightDoeFilters(groupId).then((r) => r.data.data?.values || []),
    staleTime: 5 * 60_000,
    enabled:   needsFilter && mode === 'doe',
  });

  const { data: dodFiltersRaw } = useQuery({
    ...Q,
    queryKey:  ['conf-insight-dod-filters', groupId],
    queryFn:   () => confidasApi.getInsightDodFilters(groupId).then((r) => r.data.data?.values || []),
    staleTime: 5 * 60_000,
    enabled:   needsFilter && mode === 'dod',
  });

  const filterOptions = isDos
    ? (dosFiltersRaw || [])
    : isDod
      ? (dodFiltersRaw || [])
      : (doeFiltersRaw || []);
  ...
  const { data: dosL12Raw, isLoading: dosL12Load, error: dosL12Err } = useQuery({
    ...Q,
    queryKey: ['conf-insight-dos-last12', groupId, filter],
    queryFn:  () => confidasApi.getInsightDosLast12(groupId, filter).then((r) => r.data.data?.rows || []),
    enabled:  mode === 'dos',
  });

  const { data: dosLMRaw, isLoading: dosLMLoad, error: dosLMErr } = useQuery({
    ...Q,
    queryKey: ['conf-insight-dos-lastmonth', groupId, filter],
    queryFn:  () => confidasApi.getInsightDosLastMonth(groupId, filter).then((r) => r.data.data?.rows || []),
    enabled:  mode === 'dos',
  });
  ...
  const { data: doeL12Raw, isLoading: doeL12Load, error: doeL12Err } = useQuery({
    ...Q,
    queryKey: ['conf-insight-doe-last12', groupId, filter],
    queryFn:  () => confidasApi.getInsightDoeLast12(groupId, filter).then((r) => r.data.data?.rows || []),
    enabled:  mode === 'doe',
  });

  const { data: doeLMRaw, isLoading: doeLMLoad, error: doeLMErr } = useQuery({
    ...Q,
    queryKey: ['conf-insight-doe-lastmonth', groupId, filter],
    queryFn:  () => confidasApi.getInsightDoeLastMonth(groupId, filter).then((r) => r.data.data?.rows || []),
    enabled:  mode === 'doe',
  });
  ...
  const { data: dodL12Raw, isLoading: dodL12Load, error: dodL12Err } = useQuery({
    ...Q,
    queryKey: ['conf-insight-dod-last12', groupId, filter],
    queryFn:  () => confidasApi.getInsightDodLast12(groupId, filter).then((r) => r.data.data?.rows || []),
    enabled:  mode === 'dod',
  });

  const { data: dodLMRaw, isLoading: dodLMLoad, error: dodLMErr } = useQuery({
    ...Q,
    queryKey: ['conf-insight-dod-lastmonth', groupId, filter],
    queryFn:  () => confidasApi.getInsightDodLastMonth(groupId, filter).then((r) => r.data.data?.rows || []),
    enabled:  mode === 'dod',
  });
  ...
  const retryL12 = () => queryClient.invalidateQueries({
    queryKey: isDos
      ? ['conf-insight-dos-last12', groupId, filter]
      : isDod
        ? ['conf-insight-dod-last12', groupId, filter]
        : ['conf-insight-doe-last12', groupId, filter],
  });
  const retryLM = () => queryClient.invalidateQueries({
    queryKey: isDos
      ? ['conf-insight-dos-lastmonth', groupId, filter]
      : isDod
        ? ['conf-insight-dod-lastmonth', groupId, filter]
        : ['conf-insight-doe-lastmonth', groupId, filter],
  });
