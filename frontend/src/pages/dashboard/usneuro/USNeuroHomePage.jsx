/**
 * USNeuroHomePage.jsx — US Neuro Dashboard Home tab.
 *
 * Sessions 1, 3, 4 — KPI Cards + all chart rows.
 * Layout mirrors QFDHomePage exactly:
 *
 ...
 *
 * Global filters applied in backend SQL:
 *   - Biller: remove Collective Group / Collective Bill Only
 *   - Insurance: exclude Aetna / Medicare
 *   - Date: last 12 months, >= 2025-04-01, exclude last 2 months
 */
...
export default function USNeuroHomePage({ onNavigate }) {
  ...
  const { data: kpis,        isLoading: kpiLoad       } = useQuery({ queryKey: ['usneuro-kpis'],                  queryFn: () => usneuroApi.getKpis().then(r => r.data.data),                staleTime: 300_000 });
  const { data: paymentData, isLoading: payLoad        } = useQuery({ queryKey: ['usneuro-payment-history'],       queryFn: () => usneuroApi.getPaymentHistory().then(r => r.data.data),       staleTime: 300_000 });
  const { data: paymentFull, isLoading: payFullLoad    } = useQuery({ queryKey: ['usneuro-payment-history-full'],  queryFn: () => usneuroApi.getPaymentHistoryFull().then(r => r.data.data),   staleTime: 300_000, enabled: row1Expanded === 'payment' });
  const { data: ccrData,     isLoading: ccrLoad        } = useQuery({ queryKey: ['usneuro-ccr-history'],           queryFn: () => usneuroApi.getCcrHistory().then(r => r.data.data),           staleTime: 300_000 });
  const { data: arPieData,   isLoading: arPieLoad      } = useQuery({ queryKey: ['usneuro-ar-pie'],                queryFn: () => usneuroApi.getArPie().then(r => r.data.data),                staleTime: 300_000 });
  const { data: donutData,   isLoading: donutLoad      } = useQuery({ queryKey: ['usneuro-ar-donut'],              queryFn: () => usneuroApi.getArDonut().then(r => r.data.data),              staleTime: 300_000 });
  const { data: tcData,      isLoading: tcLoad         } = useQuery({ queryKey: ['usneuro-total-charges'],         queryFn: () => usneuroApi.getTotalCharges().then(r => r.data.data),         staleTime: 300_000 });
  const { data: tcFull,      isLoading: tcFullLoad     } = useQuery({ queryKey: ['usneuro-total-charges-full'],    queryFn: () => usneuroApi.getTotalChargesFull().then(r => r.data.data),     staleTime: 300_000, enabled: row3Expanded === 'totalCharges' });
  const { data: adjData,     isLoading: adjLoad        } = useQuery({ queryKey: ['usneuro-adjustments'],           queryFn: () => usneuroApi.getAdjustments().then(r => r.data.data),          staleTime: 300_000 });
  const { data: adjFull,     isLoading: adjFullLoad    } = useQuery({ queryKey: ['usneuro-adjustments-full'],      queryFn: () => usneuroApi.getAdjustmentsFull().then(r => r.data.data),      staleTime: 300_000, enabled: row3Expanded === 'adjustments' });
  const { data: chargesData, isLoading: chargesLoad    } = useQuery({ queryKey: ['usneuro-charges-vs-payments'],   queryFn: () => usneuroApi.getChargesVsPayments().then(r => r.data.data),    staleTime: 300_000 });
  const { data: chargesFull, isLoading: chargesFullLoad} = useQuery({ queryKey: ['usneuro-charges-vs-payments-full'], queryFn: () => usneuroApi.getChargesVsPaymentsFull().then(r => r.data.data), staleTime: 300_000, enabled: row2Expanded === 'charges' });
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={activeAdj || []} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="usneuroAdjGrad" x1="0" y1="0" x2="0" y2="1">
