...
export default function USNeuroCleanClaimPage({ onNavigate }) {
  const { data: kpis,       isLoading: kpiLoading    } = useQuery({
    queryKey: ['usneuro-kpis'],
    queryFn:  () => usneuroApi.getKpis().then(r => r.data.data),
    staleTime: 300_000,
  });
  const { data: ccrData,    isLoading: ccrLoading    } = useQuery({
    queryKey: ['usneuro-ccr-history'],
    queryFn:  () => usneuroApi.getCcrHistory().then(r => r.data.data),
    staleTime: 300_000,
  });
  const { data: denialData, isLoading: denialLoading } = useQuery({
    queryKey: ['usneuro-denial-reasons'],
    queryFn:  () => usneuroApi.getDenialReasons().then(r => r.data.data),
    staleTime: 300_000,
  });
