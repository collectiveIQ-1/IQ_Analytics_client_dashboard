/**
 * USNeuroProductionPage.jsx — Productions tab for US Neuro Dashboard.
 *
 ...
 *   DOE — Single blue bar chart: Total Charges by Date of Entry
 *         More → Reimbursement Analysis table (grouped by DOE month)
 *   DOD — 2×2 grid:
 *         Top-left:  Last 12 Months Adjustment History (area)
 *         Top-right: Last 12 Months Payments by Payer Wise (h-bar)
 *         Bot-left:  Last 12 Months Payments History (line)
 *         Bot-right: Last 12 Months Payments by Biller Entity (h-bar)
 */
...
// ── Reimbursement row definitions (same as QFD) ───────────────────────────────
...
const REIMB_ROWS = [
  ...
  { key: 'gcr_fully_paid',         label: 'GCR based on Fully Paid Cl..',   fmt: fmtPct0,   indent: 0 },
...
function ReimbursementTable({ data, loading }) {
  if (loading) {
      ...
      <table className="min-w-full text-xs">
        <thead>
          <tr className="border-b border-slate-200 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-900">
            <th className="sticky left-0 z-10 bg-slate-50 dark:bg-zinc-900 text-left px-4 py-2.5 font-semibold text-slate-600 dark:text-zinc-300 min-w-[220px]">
              Metric
            </th>
            {data.map((row, i) => (
...
export default function USNeuroProductionPage() {
  ...
  const { data: dosChart, isLoading: dosLoad } = useQuery({
    queryKey: ['usneuro-prod-dos-chart'],
    queryFn:  () => usneuroApi.getProductionDosChart().then(r => r.data.data),
    staleTime: 300_000,
    enabled: view === 'dos' && !showMore,
  });

  const { data: doeChart, isLoading: doeLoad } = useQuery({
    queryKey: ['usneuro-prod-doe-chart'],
    queryFn:  () => usneuroApi.getProductionDoeChart().then(r => r.data.data),
    staleTime: 300_000,
    enabled: view === 'doe' && !showMore,
  });

  const { data: dosTable, isLoading: dosTblLoad } = useQuery({
    queryKey: ['usneuro-prod-dos-reimb'],
    queryFn:  () => usneuroApi.getProductionDosReimb().then(r => r.data.data),
    staleTime: 300_000,
    enabled: view === 'dos' && showMore,
  });

  const { data: doeTable, isLoading: doeTblLoad } = useQuery({
    queryKey: ['usneuro-prod-doe-reimb'],
    queryFn:  () => usneuroApi.getProductionDoeReimb().then(r => r.data.data),
    staleTime: 300_000,
    enabled: view === 'doe' && showMore,
  });

  const { data: adjData,    isLoading: adjLoad    } = useQuery({
    queryKey: ['usneuro-prod-dod-adj'],
    queryFn:  () => usneuroApi.getAdjustments().then(r => r.data.data),
    staleTime: 300_000,
    enabled: view === 'dod',
  });

  const { data: pmtData,    isLoading: pmtLoad    } = useQuery({
    queryKey: ['usneuro-prod-dod-pmt'],
    queryFn:  () => usneuroApi.getPaymentHistory().then(r => r.data.data),
    staleTime: 300_000,
    enabled: view === 'dod',
  });

  const { data: payerData,  isLoading: payerLoad  } = useQuery({
    queryKey: ['usneuro-prod-dod-payer'],
    queryFn:  () => usneuroApi.getProductionDodByPayer().then(r => r.data.data),
    staleTime: 300_000,
    enabled: view === 'dod',
  });

  const { data: billerData, isLoading: billerLoad } = useQuery({
    queryKey: ['usneuro-prod-dod-biller'],
    queryFn:  () => usneuroApi.getProductionDodByBiller().then(r => r.data.data),
    staleTime: 300_000,
    enabled: view === 'dod',
  });
        ...
        {(view === 'dos' || view === 'doe') && showMore && (
          <>
            <div className="px-4 pb-2">
              <ReimbursementTable data={activeTable} loading={tableFetching} />
            </div>
            <div className="flex justify-end px-4 pb-4">
        ...
        {view === 'dod' && (
          <div className="px-4 pb-4 space-y-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {adjLoad ? <ChartSkeleton height={240} /> : (
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={adjData || []} margin={{ top: 28, right: 20, left: 10, bottom: 10 }}>
                        ...
                        <XAxis dataKey="date" tickFormatter={fmtShortDate}
                          tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <YAxis tickFormatter={fmtMoney} width={52}
                          tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <Tooltip content={<ChartTooltip />} />
                        <Area type="monotone" dataKey="adjustments" name="Adjustments"
                          stroke="#D97706" strokeWidth={2} fill="url(#usneuroAdjGrad)" dot={false}>
              ...
              {/* Payments by Payer Wise — horizontal bar */}
              <div>
                <p className="text-[11px] font-semibold text-slate-600 dark:text-zinc-300 mb-1 px-1">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        <XAxis type="number" tickFormatter={fmtMoney}
                          tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <YAxis dataKey="payer" type="category" width={130}
                  ...
                  {billerLoad ? <ChartSkeleton height={240} /> : (
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        <XAxis type="number" tickFormatter={fmtMoney}
                          tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
                        <YAxis dataKey="biller_entity" type="category" width={140}
                          tick={{ fontSize: 9, fill: '#334155', fontWeight: 600 }}
                          axisLine={false} tickLine={false} />
                        <Tooltip content={<ChartTooltip />} />
