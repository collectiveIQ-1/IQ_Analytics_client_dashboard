/**
 * CompleteNeuroHomePage.jsx — Complete Neuro Dashboard Home tab.
 *
 * Layout mirrors IOMHelpHomePage exactly:
...
export default function CompleteNeuroHomePage({ onNavigate }) {
  ...
  const { data: kpis,        isLoading: kpiLoad        } = useQuery({ queryKey: ['cn-kpis'],                  queryFn: () => completeneuroApi.getKpis().then(r => r.data.data),                staleTime: 300_000 });
  const { data: paymentData, isLoading: payLoad         } = useQuery({ queryKey: ['cn-payment-history'],       queryFn: () => completeneuroApi.getPaymentHistory().then(r => r.data.data),       staleTime: 300_000 });
  const { data: paymentFull, isLoading: payFullLoad     } = useQuery({ queryKey: ['cn-payment-history-full'],  queryFn: () => completeneuroApi.getPaymentHistoryFull().then(r => r.data.data),   staleTime: 300_000, enabled: row1Expanded === 'payment' });
  const { data: ccrData,     isLoading: ccrLoad         } = useQuery({ queryKey: ['cn-ccr-history'],           queryFn: () => completeneuroApi.getCcrHistory().then(r => r.data.data),           staleTime: 300_000 });
  const { data: arPieData,   isLoading: arPieLoad       } = useQuery({ queryKey: ['cn-ar-pie'],                queryFn: () => completeneuroApi.getArPie().then(r => r.data.data),                staleTime: 300_000 });
  const { data: donutData,   isLoading: donutLoad       } = useQuery({ queryKey: ['cn-ar-donut'],              queryFn: () => completeneuroApi.getArDonut().then(r => r.data.data),              staleTime: 300_000 });
  const { data: tcData,      isLoading: tcLoad          } = useQuery({ queryKey: ['cn-total-charges'],         queryFn: () => completeneuroApi.getTotalCharges().then(r => r.data.data),         staleTime: 300_000 });
  const { data: tcFull,      isLoading: tcFullLoad      } = useQuery({ queryKey: ['cn-total-charges-full'],    queryFn: () => completeneuroApi.getTotalChargesFull().then(r => r.data.data),     staleTime: 300_000, enabled: row3Expanded === 'totalCharges' });
  const { data: adjData,     isLoading: adjLoad         } = useQuery({ queryKey: ['cn-adjustments'],           queryFn: () => completeneuroApi.getAdjustments().then(r => r.data.data),          staleTime: 300_000 });
  const { data: adjFull,     isLoading: adjFullLoad     } = useQuery({ queryKey: ['cn-adjustments-full'],      queryFn: () => completeneuroApi.getAdjustmentsFull().then(r => r.data.data),      staleTime: 300_000, enabled: row3Expanded === 'adjustments' });
  const { data: chargesData, isLoading: chargesLoad     } = useQuery({ queryKey: ['cn-charges-vs-payments'],   queryFn: () => completeneuroApi.getChargesVsPayments().then(r => r.data.data),    staleTime: 300_000 });
  const { data: chargesFull, isLoading: chargesFullLoad } = useQuery({ queryKey: ['cn-charges-vs-payments-full'], queryFn: () => completeneuroApi.getChargesVsPaymentsFull().then(r => r.data.data), staleTime: 300_000, enabled: row2Expanded === 'charges' });
          ...
          >
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={activeAdj || []} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                <defs>
                  <linearGradient id="cnAdjGrad" x1="0" y1="0" x2="0" y2="1">
