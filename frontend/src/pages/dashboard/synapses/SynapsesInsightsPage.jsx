/**
 * SynapsesInsightsPage.jsx — Insights tab for IOM Help Dashboard.
 *
 * Toggle: Technician | Reader | Insurance | Surgeon
...
const fmtCount = (v) => { const n = Number(v||0); return n === 0 ? '' : '$' + (n/1000).toFixed(2) + 'K'; };
...
function InsightsTable({ data, loading, entityLabel, title }) {
...
export default function SynapsesInsightsPage() {
  ...
  const { data: insuranceData, isLoading: insLoad } = useQuery({
    queryKey: ['synm-insights-insurance'],
    queryFn:  () => synapsesApi.getInsightsInsurance().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    enabled: view === 'insurance',
  });
  const { data: surgeonData, isLoading: surLoad } = useQuery({
    queryKey: ['synm-insights-surgeon', filter],
    queryFn:  () => synapsesApi.getInsightsSurgeon(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    enabled: view === 'surgeon',
  });
  const { data: readerData, isLoading: rdLoad } = useQuery({
    queryKey: ['synm-insights-reader', filter],
    queryFn:  () => synapsesApi.getInsightsReader(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
    enabled: view === 'reader',
  });
  const { data: techData, isLoading: techLoad } = useQuery({
    queryKey: ['synm-insights-technician', filter],
    queryFn:  () => synapsesApi.getInsightsTechnician(filter).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  ...
  const { data: surgeonList = [] } = useQuery({
    queryKey: ['synm-insights-surgeon-list'],
    queryFn:  () => synapsesApi.getInsightsSurgeonList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    enabled: view === 'surgeon',
  });
  const { data: readerList = [] } = useQuery({
    queryKey: ['synm-insights-reader-list'],
    queryFn:  () => synapsesApi.getInsightsReaderList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    enabled: view === 'reader',
  });
  const { data: techList = [] } = useQuery({
    queryKey: ['synm-insights-tech-list'],
    queryFn:  () => synapsesApi.getInsightsTechList().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 30 * 60_000,
    enabled: view === 'technician',
  });
  ...
  const title       = view === 'insurance'  ? 'Insurance Wise Analysis'
                    ...
                    : 'Collector Wise Analysis';
  const dropLabel   = view === 'surgeon'    ? 'Surgeon'
                    : view === 'reader'     ? 'Reader'
                    : view === 'technician' ? 'Collector'
                    : null;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-1.5 flex-wrap">
          <ViewBtn active={view === 'technician'} onClick={() => switchView('technician')}>Collector</ViewBtn>
