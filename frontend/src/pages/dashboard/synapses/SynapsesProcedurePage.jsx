/**
 * SynapsesProcedurePage.jsx — Procedure tab for IOM Help Dashboard.
 *
 * Views (toggled by DOS / DOE buttons top-right):
 *   DOS — Two side-by-side horizontal bar charts
 *         Left:  Deposits by Procedure Type  (payment_report / billing_type)
 *         Right: Charges by Procedure Type   (billing_report_iomhelp / procedure_type / dos)
 *         More → Charges by Procedure Type detailed table with DOS/DOE inner toggle
 *   DOE — Same layout, Charges use billing_date
 *         More → Same detail table (DOE mode selected by default)
 *
 * Schema: iq_ionm
 * Tables:
 *   payment_report         → Deposits by Procedure Type
 *   billing_report_iomhelp → Charges by Procedure Type + DOS/DOE More
 */
...
function HBarCard({ title, data, loading, color, onMore, barKey = 'value' }) {
        ...
        {loading ? (
          ...
          <ResponsiveContainer width="100%" height={barHeight}>
            ...
            >
              ...
              <Bar dataKey={barKey} name="Amount" fill={color} radius={[0, 3, 3, 0]}>
                <LabelList dataKey={barKey} content={HBarLabel} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        )}
                ...
                DOD_METRIC_ROWS.map((row, rowIdx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {months.map((m) => {
                        const cellData = monthMap[m];
                        const val = cellData ? cellData[row.key] : 0;
                        return (
...
function ProcedureMoreTable({ dosData, doeData, dosLoading, doeLoading, initialMode, onGoBack }) {
  ...
  return (
    <div>
      {/* Header: title + DOS/DOE inner toggle */}
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[13px] font-semibold text-slate-700 dark:text-zinc-200">
...
export default function SynapsesProcedurePage() {
  ...
  const { data: depositsRaw, isLoading: depLoad } = useQuery({
    queryKey: ['synm-proc-deposits'],
    queryFn:  () => synapsesApi.getProcedureDeposits().then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  });

  const { data: chargesRaw, isLoading: chrLoad } = useQuery({
    queryKey: ['synm-proc-charges', view],
    queryFn:  () => synapsesApi.getProcedureCharges(view).then((r) => r.data?.data ?? r.data ?? []),
    staleTime: 5 * 60_000,
  });

  // Pre-fetch both DOS and DOE charges-more data so the inner toggle is instant
  const { data: dosMoreRaw, isLoading: dosMoreLoad, isFetching: dosMoreFetch } = useQuery({
    queryKey: ['synm-proc-more', 'dos'],
    queryFn:  () => synapsesApi.getProcedureMore('dos').then((r) => r.data?.data ?? r.data ?? []),
    enabled:  showMore === 'charges',
    staleTime: 5 * 60_000,
  });

  const { data: doeMoreRaw, isLoading: doeMoreLoad, isFetching: doeMoreFetch } = useQuery({
    queryKey: ['synm-proc-more', 'doe'],
    queryFn:  () => synapsesApi.getProcedureMore('doe').then((r) => r.data?.data ?? r.data ?? []),
    enabled:  showMore === 'charges',
    staleTime: 5 * 60_000,
  });

  // Deposit More (DOD) data
  const { data: dodMoreRaw, isLoading: dodMoreLoad, isFetching: dodMoreFetch } = useQuery({
    queryKey: ['synm-proc-dod-more'],
    queryFn:  () => synapsesApi.getProcedureDodMore().then((r) => r.data?.data ?? r.data ?? []),
