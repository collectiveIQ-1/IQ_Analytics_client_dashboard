/**
 * SynapsesArPage.jsx — Accounts Receivable tab for IOM Help Dashboard.
 *
 ...
 *
 * Schema: iq_ionm  Table: billing_report_iomhelp
 */
...
export default function SynapsesArPage() {
  ...
  const { data: dosData, isLoading: dosLoad } = useQuery({
    queryKey: ['synm-ar-dos'],
    queryFn:  () => synapsesApi.getArDos().then((r) => r.data.data),
    staleTime: 300_000,
    enabled: view === 'dos',
  });

  const { data: doeData, isLoading: doeLoad } = useQuery({
    queryKey: ['synm-ar-doe'],
    queryFn:  () => synapsesApi.getArDoe().then((r) => r.data.data),
    staleTime: 300_000,
    enabled: view === 'doe',
  });

  const { data: insData, isLoading: insLoad } = useQuery({
    queryKey: ['synm-ar-insurance', view],
    queryFn:  () => synapsesApi.getArInsurance(view).then((r) => r.data.data),
    staleTime: 300_000,
    enabled: expanded && expandTab === 'insurance',
  });

  const { data: surgData, isLoading: surgLoad } = useQuery({
    queryKey: ['synm-ar-surgeon', view],
    queryFn:  () => synapsesApi.getArSurgeon(view).then((r) => r.data.data),
    staleTime: 300_000,
  ...
  const treemapData = useMemo(() => {
    ...
    if (activeCarrier) {
      // Keep all but dim non-active — handled via prop
    }
    return Object.entries(totals)
      .map(([name, value]) => ({ name, value }))
      .sort((a, b) => b.value - a.value);
  }, [filteredByCarrier, activeCarrier]);
  ...
  const totalAr = (raw?.buckets || []).reduce((s, r) => s + r.total_ar, 0);
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      ...
                      style={{ cursor: 'pointer' }}
                      label={({ cx: lx, cy: ly, midAngle, innerRadius: ir, outerRadius: or, percent, value }) => {
                        ...
                        const v = value >= 1000000 ? `$${(value/1000000).toFixed(1)}M`
                                ...
                                : `$${value}`;
                        return (
                          <text x={x} y={y} fill="#374151" textAnchor={x > lx ? 'start' : 'end'} dominantBaseline="central" fontSize={9} fontWeight="600">
